...
char* index_to_ngram(unsigned long long index) {

    char* ngram = new char [ngramLength];
    unsigned long long indexCounter = index;

    for (unsigned i{ }; i < ngramLength; i++) {
        ngram[i] = indexCounter / pow(LayoutSize, ngramLength - 1 - i);
        indexCounter %= (unsigned long long)pow(LayoutSize, ngramLength - 1 - i);
    }
...
void corpusRead(string corpusFile) {
    ...
    fs::path corpusDir = fs::current_path() / "corpus";
    ifstream corpus (corpusDir / corpusFile, ios::in | ios::ate);
    if (corpus.is_open()) {
        size = corpus.tellg();
        long memsize = size;
...
void corpusRecord(string corpusDataFile) {
    fs::path corpusDir = fs::current_path() / "corpus";
    ofstream corpusData(corpusDir / corpusDataFile);
...
void corpusLoad(string corpusDataFile) {
    ...
    fs::path corpusDir = fs::current_path() / "corpus";
    ifstream corpusData(corpusDir / corpusDataFile);
